...
namespace Towers.TargetingStrategies
{
    ...
    {
        [Header("Settings")]
        [SerializeField] private float yPivotSpeed = 10f; 
        ...
        public void UpdateRotation(TowerEntity tower, float deltaTime)
        {
            ...
            if (tower.yPivot)
            {
                ...
                Vector3 directionToTarget = targetPos - tower.yPivot.position;
                Vector3 planarDirection = Vector3.ProjectOnPlane(directionToTarget, Vector3.up);

                if (planarDirection.sqrMagnitude > 0.001f)
                {
                    Quaternion targetRot = Quaternion.LookRotation(planarDirection);
                    ...
                    float angle = Quaternion.Angle(tower.yPivot.rotation, targetRot);
            ...
            if (tower.xPivot)
            {
                ...
                Vector3 directionToTarget = targetPos - tower.xPivot.position;
                
                if (directionToTarget.sqrMagnitude > 0.001f)
                {
                    // Calculate Local Rotation needed
                    // Use the Y-Pivot's Up vector to ensure we don't roll
                    Vector3 upAxis = tower.yPivot ? tower.yPivot.up : Vector3.up;
                    Quaternion lookRot = Quaternion.LookRotation(directionToTarget, upAxis);
                    ...
                    float angle = Vector3.Angle(tower.xPivot.forward, directionToTarget);
                    xAligned = angle < lockThreshold;
                }
            }

            // 4. Update Status
            bool currentLock = yAligned && xAligned;
