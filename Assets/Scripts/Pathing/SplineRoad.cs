...
namespace Pathing
{
    ...
    public class Intersection
    {
        ...
        {
            if (Curves == null) Curves = new List<float>();
            Curves.Add(value);
        }
        ...
        {
            if (Junctions == null) Junctions = new List<JunctionInfo>();
    ...
    public class SplineRoad : MonoBehaviour
    {
        ...
        [SerializeField] [Min(0)] private int _RoadResolution;

        [SerializeField] [Min(0f)] private float _RoadWidth;

        [SerializeField] [Min(1)] private int _CurveSteps;

        [SerializeField] private List<SplineRoadOverrideSettings> _SplineOverridesVector;
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            for (var i = 0; i < _rightPoints.Count; i++)
            {
        ...
        {
            if (_currentCurveSteps != _CurveSteps || _currentResolution != _RoadResolution ||
                !Mathf.Approximately(_currentRoadWidth, _RoadWidth)) RebuildMesh();
        }

        public void AddIntersections(Intersection intersection)
        {
            if (_intersections == null) _intersections = new List<Intersection>();
        ...
        public SplineRoadOverrideSettings GetSettings(int splineIndex)
        {
            ...
            if (!_splineOverrides.ContainsKey(splineIndex))
            {
                ...
                if (!found) _SplineOverridesVector.Add(newOverride);
            }
        ...
        public void RebuildMesh()
        {
            ...
            if (!_meshCollider)
            {
                ...
                if (_meshCollider) _meshCollider.sharedMesh = _meshFilter.sharedMesh;
            }
            else
            {
        ...
        private void GetVertices()
        {
            ...
            for (var j = 0; j < _SplineSampler.NumSplines; j++)
            {
                var splineOverride = GetSettings(j);
                var step = 1f / splineOverride.RoadResolution;

                for (var i = 0; i < splineOverride.RoadResolution; i++)
                {
                    var t = step * i;
        ...
        private void GenerateRoadSubMesh(List<Vector3> verts, List<int> tris, List<Vector2> uvs)
        {
            var offset = 0;
            var splineOffset = 0;
            for (var currentSplineIndex = 0; currentSplineIndex < _SplineSampler.NumSplines; currentSplineIndex++)
            {
                var splineOverride = GetSettings(currentSplineIndex);

                var uvOffset = 0f;
                for (var currentSplinePoint = 1;
                     currentSplinePoint < splineOverride.RoadResolution + 1;
                     currentSplinePoint++)
                {
                    var vertOffset = splineOffset + currentSplinePoint + currentSplineIndex;
                    var p1 = _rightPoints[vertOffset - 1];
                    var p2 = _leftPoints[vertOffset - 1];
                    var p3 = _rightPoints[vertOffset];
                    var p4 = _leftPoints[vertOffset];
                    ...
                    var distance = Vector3.Distance(p1, p3) / 4f;
                    var uvDistance = uvOffset + distance;
        ...
        private void GenerateIntersectionsSubMesh(List<Vector3> verts, List<int> tris, List<Vector2> uvs)
        {
            if (_intersections != null)
                ...
                {
                    var intersection = _intersections[i];
                    var count = 0;
                    // List<Vector3> points = new List<Vector3>();
                    var junctionEdges = new List<JunctionEdge>();
                    ...
                    for (var j = 1; j <= junctionEdges.Count; j++)
                    {
                    ...
                    var pointsOffset = verts.Count;
                    for (var j = 1; j <= curvePoints.Count; j++)
                    {
                        verts.Add(center);
        ...
        private int SortPoints(Vector3 center, Vector3 x, Vector3 y)
        {
            ...
            var angleA = Vector3.SignedAngle(center.normalized, xDir.normalized, Vector3.up);
            var angleB = Vector3.SignedAngle(center.normalized, yDir.normalized, Vector3.up);
