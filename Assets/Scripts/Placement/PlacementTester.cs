...
namespace Placement
{
    public class PlacementTester : MonoBehaviour
    {
        ...
        [Header("Test Data")]
        [Tooltip("Glisse tes Blueprints de tours ici pour les tester")]
        [SerializeField] private List<BuildingSo> testTowers;

        private void Update()
        {
            // Raccourcis Clavier pour tester rapidement
            // Touche '1' pour la première tour, '2' pour la deuxième, etc.
            if (testTowers.Count > 0 && UnityEngine.InputSystem.Keyboard.current.digit1Key.wasPressedThisFrame)
            {
                SelectTower(0);
            }
            if (testTowers.Count > 1 && UnityEngine.InputSystem.Keyboard.current.digit2Key.wasPressedThisFrame)
            {
                SelectTower(1);
            }
            if (testTowers.Count > 2 && UnityEngine.InputSystem.Keyboard.current.digit3Key.wasPressedThisFrame)
            {
                SelectTower(2);
            }
