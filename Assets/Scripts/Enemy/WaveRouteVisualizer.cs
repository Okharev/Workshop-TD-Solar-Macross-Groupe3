    ...
    public class WaveRouteVisualizer : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (waveManager != null)
            {
                ...
                if (!waveManager.IsWaveActive) ShowNextRoutes();
            }
        }
        ...
        private void ShowNextRoutes()
        {
            ...
            var nextWaveIndex = waveManager.CurrentWaveIndex + 1;
        ...
        private void DrawGroundRoutes(WaveProfile wave)
        {
            ...
            foreach (var roadIndex in wave.unlockedRoadIndices)
            {
                if (roadIndex < 0 || roadIndex >= container.Splines.Count) continue;

                var spline = container.Splines[roadIndex];

                // Création de l'objet Ligne
                var lineObj = CreateLineObject($"GroundRoute_{roadIndex}", groundColor, groundLineMaterial);
                ...
                for (float t = 0; t <= 1.0001f; t += splineResolution)
                {
                    ...
                    var worldPos = container.transform.TransformPoint(spline.EvaluatePosition(t));
        ...
        private void DrawAirRoutes(WaveProfile wave)
        {
            ...
            foreach (var segment in wave.airSegments)
            {
                ...
                var path = segment.targetPath;

                // Création de l'objet Ligne
                var lineObj = CreateLineObject($"AirRoute_{path.name}", airColor, airLineMaterial);
                var lr = lineObj.GetComponent<LineRenderer>();
                ...
                if (finalTarget.HasValue) points.Add(finalTarget.Value);
