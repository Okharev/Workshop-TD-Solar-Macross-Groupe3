    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes input)
            {
                ...
                float sy, cy; sincos(angleY, sy, cy);
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                // We use the random seed (rnd) to mix between Color A and Color B.
                // rnd goes from 0 to 1, so every flower gets a unique blend.
                float4 petalColor = lerp(_TipColorA, _TipColorB, input.rnd);
                
                // --- 2. MIX STEM AND PETAL ---
                // We use smoothstep to create a soft transition from Green Stem to Colored Petal.
                // _PetalStart controls where the color begins (e.g. 0.6 means top 40% is colored).
